...
if (isset($_SESSION['ssn_id'])) {
    ...
    if ($id==$current_id) {
       header("location:profile.php");
        ...
        $query_for_check_user=mysqli_query($conn,"SELECT * FROM users WHERE unique_id='$id' ");
        if (mysqli_num_rows($query_for_check_user) == 0 ) {
            ...
            $query_crnusr=mysqli_query($conn,"SELECT * FROM users WHERE unique_id='$current_user' ");
            $fetchcrnusr=mysqli_fetch_assoc($query_crnusr);
            $query=mysqli_query($conn,"SELECT * FROM users WHERE unique_id='$id' ");
            $fetch=mysqli_fetch_assoc($query);
            $name= $fetch['name'];
            $profile='images/'.$fetch['profile'];
            $bio=$fetch['bio'];
            $crname= $fetchcrnusr['name'];
            $crprofile='images/'.$fetchcrnusr['profile'];
            $crbio=$fetchcrnusr['bio'];
    ...
    <div class="main_wrapper">
        ...
        <div class="navbar_main">
            <div class="nav_header">
                <img src="<?php echo $crprofile; ?>" alt="">
                <div class="text">
                    <span class="name">
                    <?php echo $crname; ?>
                    </span>
        ...
        <div class="profile_main_div">
            ...
            <div class="questions_container profile" style="positation:relative">
                        <?php
                        $question_query=mysqli_query($conn,"SELECT * FROM questions WHERE owner='$id' ORDER BY id DESC");
                        $all_userquery=mysqli_query($conn,"SELECT * FROM users ORDER BY id DESC");
                        $fetch_all_user=mysqli_fetch_assoc($all_userquery);
                        $post_count=mysqli_num_rows($question_query);
                        if ($question_query) {
                            echo "<h4>".$fetch['name']."'s have $post_count Posts</h4>";
